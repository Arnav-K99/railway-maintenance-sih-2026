        ...
        <div className="min-w-[960px]">
          ...
          <div className="divide-y divide-slate-100">
            {sections.map((sec) => {
              ...
              const rowHeight = isMultiTask ? 'h-20' : 'h-14';

              return (
                <div
                  key={sec.section_id}
                  className={`flex items-center group hover:bg-slate-50/70 transition-colors ${rowHeight}`}
                >
                  {/* Section Label */}
                  <div className="w-36 shrink-0 px-3 py-2 border-r border-slate-200 font-mono text-xs flex flex-col justify-center h-full">
                    <div className="flex items-center justify-between">
                      <span className="font-bold text-slate-800">{sec.section_id}</span>
                      {tasks.some((t) => t.is_bundled) && (
                        <span className="text-[9px] bg-purple-100 text-purple-800 font-bold px-1 rounded uppercase font-sans">
                          Bundled
                        </span>
                      )}
                    </div>
                  ...
                  <div className="flex-1 h-full relative grid grid-cols-12 divide-x divide-slate-100/80">
                    ...
                    {tasks.map((task, taskIdx) => {
                      ...
                      // Dynamic vertical positioning for stacked concurrent / bundled tasks
                      const posClasses = isMultiTask
                        ? taskIdx === 0
                          ? 'top-1.5 h-8'
                          : 'top-10 h-8'
                        : 'top-2 bottom-2';
                      ...
                      return (
                        <div
                          ...
                          className={`absolute ${posClasses} rounded-md shadow-xs border px-2 py-0.5 cursor-pointer transition-all hover:scale-[1.01] hover:shadow-md flex flex-col justify-center overflow-hidden z-10 ${barColor}`}
                          title={`${task.task_id} | ${task.maintenance_type || task.department} | ${task.duration_minutes} min | ${task.department}`}
                        >
                          <div className="flex items-center justify-between text-[10px] font-bold leading-tight truncate">
                            <span className="font-mono truncate">{task.task_id}</span>
                            <span className="text-[8px] bg-black/30 px-1 py-0.2 rounded uppercase tracking-wider shrink-0 ml-1">
                              {badgeType}
                            </span>
                          </div>
                          <div className="text-[9px] opacity-90 truncate leading-tight">
                            {task.maintenance_type ? `${task.maintenance_type} (${task.duration_minutes}m)` : `${task.duration_minutes}m • ${task.department.split('/')[0]}`}
                          </div>
                        </div>
                      );
                    })}
